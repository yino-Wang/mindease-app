...
export function TopPickHeroCard({ item }: TopPickHeroCardProps) {
  return (
    ...
    >
      <article
        className={`relative aspect-[21/9] max-h-[420px] w-full overflow-hidden sm:aspect-[2.4/1] ${CARD_RADIUS_LG} ${CARD_SURFACE} ${CARD_BORDER} transition-opacity duration-700 ease-in-out motion-reduce:transition-none group-hover:opacity-95`}
      >
        {item.videoUrl ? (
          <SafeAmbientVideo
            src={item.videoUrl}
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-black/75 via-black/25 to-stone-950/10" />
